...
export class UsulancreatePage {
  ...
  uploadFile() {
    ...
    fileTransfer.upload(this.imageURI, 'http://sedesa.id/api/upload', options)
      .then((data) => {
  ...
  uploadPhotoProposal() {
    ...
    fileTransfer.upload(this.fileURI, 'http://sedesa.id/api/uploadproposal', options)
      .then((data) => {
  ...
  downloadImage(photos_awal) {
    photos_awal.forEach(element => {
      this.platform.ready().then(() => {
        ...
        const imageLocation = `http://sedesa.id/fotoupload/${element}`;
  ...
  downloadFile(files_awal) {
    files_awal.forEach(element => {
      this.platform.ready().then(() => {
        const fileTransfer:FileTransferObject = this.transfer.create();
        //const imageLocation = `${this.file.applicationDirectory}www/assets/img/${element}`;
        const imageLocation = `http://sedesa.id/fileupload/${element}`;
